...
use anyhow::anyhow;
use icelake::io::task_writer::TaskWriter;
use icelake::Table;
...
impl IcebergSink {
    ...
    pub async fn new(config: IcebergConfig, schema: Schema) -> Result<Self> {
        ...
        let table = Table::open_with_op(op).await?;
...
impl Sink for IcebergSink {
    ...
    async fn new_writer(&mut self, _writer_param: SinkWriterParam) -> Result<Self::Writer> {
        // TODO: check consistency between schema and table schema.
        self.table
            .task_writer()
            .await
            .map_err(|e| SinkError::Iceberg(format!("{}", e)))
    }
}

#[async_trait::async_trait]
impl SinkWriter for TaskWriter {
